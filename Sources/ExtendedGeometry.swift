...
struct ExtendedGeometry {
    ...
    init(data: Data, previous:ExtendedGeometry?=nil) throws {
        ...
        guard data.prefix(4) == Data("MGE5".utf8), try bytes.i32(4) == 5 else {
        ...
        let counts=try (0..<7).map { try bytes.i32(28+$0*4) }, strides=[8,24,36,140,16,12,24]
        var expected=120
            ...
            if equal(12,4),equal(28,92),equal(offsets[0],offsets[2]-offsets[0]),
               equal(offsets[4],data.count-offsets[4]) {
                ...
                for i in sectors.indices {
                    let p=offsets[3]+i*140
                    if !equal(p,140) { sectors[i]=try sector(p) }
                }
