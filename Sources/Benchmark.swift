...
extension App {
    ...
    @objc func validateMenuItem(_ item: NSMenuItem) -> Bool {
        ...
        if item.action == #selector(toggleMinimalHUDPortrait) { item.state=renderer.minimalHUDPortrait ? .on:.off }
        if item.action == #selector(selectHUDStyle(_:)) { item.state=renderer.hudStyle.rawValue==item.tag ? .on:.off }
