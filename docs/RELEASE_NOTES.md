...
The current local development app is **0.10.0 build 129**, ad-hoc signed and
...
Final successful local build: **129**.

Build 129 adds [copied engine geometry](EXTENDED_GEOMETRY.md) to the separate
[native simulation worker](EXTENDED_ENGINE.md), with shared 32-bit BSP references
and native mesh checks on all sixteen Rust maps, including MAP13 XNOD. The worker
also has ordered session planning, required ID24 fields and map/weapon probes.
