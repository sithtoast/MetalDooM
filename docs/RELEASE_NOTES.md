...
The current local development app is **0.10.0 build 131**, ad-hoc signed and
...
Final successful local build: **131**.

Build 131 adds an explicit [Rust world preview](EXTENDED_PREVIEW.md), with a
separate worker process, native textured worlds/skies and manual simulation
controls. Actor/weapon/audio presentation and full gameplay remain ahead; the
normal picker keeps its Rust guard. All sixteen maps pass textured-scene checks.
