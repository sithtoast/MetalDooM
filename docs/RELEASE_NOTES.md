...
The current local development app is **0.10.0 build 132**, ad-hoc signed and
...
Final successful local build: **132**.

Build 132 extends the explicit [Rust preview](EXTENDED_PREVIEW.md) with native
actor and weapon sprites, camera-relative rotations, firing controls and ammo
status. All sixteen maps pass scene/frame decoding checks; actual Rust weapon
probes cover firing and projectile artwork. Audio, palette/translucency effects
and full gameplay remain ahead; the normal picker keeps its Rust guard.
The worker also has ordered sessions, required ID24 fields and combat probes.
