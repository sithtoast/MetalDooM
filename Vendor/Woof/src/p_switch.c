...
//
// MetalDooM: GPL vanilla defaults from the vendored Chocolate Doom source.
static switchlist_t native_switches[] =
{
...
void P_InitSwitchList(void)
{
  ...
  int native_lump = W_CheckNumForName("SWITCHES");
  alphSwitchList = native_lump < 0 ? native_switches : W_CacheLumpNum(native_lump, PU_STATIC);
  ...
  numswitches = index/2;
  switchlist[index] = -1;
  if (native_lump >= 0) Z_ChangeTag(alphSwitchList,PU_CACHE); //jff 3/23/98 allow table to be freed
}
