...
boolean M_FileExistsNotDir(const char *path)
{
    ...
    return (MEFS_GetPathInfo(path, &info)
            && info.type != MEFS_PATHTYPE_DIRECTORY);
}
...
int M_FileLength(const char *path)
{
    ...
    if (!MEFS_GetPathInfo(path, &info))
    {
        I_Error("MEFS_GetPathInfo: %s", MEFS_GetError());
    }
...
int64_t M_FileMTime(const char *path)
{
    ...
    return (int64_t)MEFS_NS_TO_SECONDS(info.modify_time);
}
...
{
    boolean success = MEFS_SaveFile(name, source, length);

    if (!success)
    {
        M_remove(name); // Remove partially written file
        I_Printf(VB_ERROR, "Couldn't write file %s: %s", name, MEFS_GetError());
    }
