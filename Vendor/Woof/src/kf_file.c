...
#include "s_musinfo.h"
#include "p_enemy.h"
#include "NativeSaveValidation.h"
...
{
    if (index >= 0 && (index >= array_size(thinker_pointers) ||
        (thinker_pointers[index].tc != tc_mobj && thinker_pointers[index].tc != tc_mobj_del)))
        I_Error("Save reference is not an actor");
    if (index == head_index) I_Error("Invalid actor sentinel");
...
static void UnArchiveDirty(json_t *root)
{
    ...
    array_foreach_type(dl, dirty_lines, dirty_line_t)
    {
        ...
        ME_CheckSaveIndex(JS_GetIntegerValue(line_obj,"line"),"lines");
    ...
    array_foreach_type(ds, dirty_sides, dirty_side_t)
    {
        ...
        ME_CheckSaveIndex(JS_GetIntegerValue(side_obj,"side"),"sides");
...
static void UnArchiveWorld(json_t *root)
{
    ...
    for (i = 0, line = lines; i < numlines; i++, line++)
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            {
                if (line->sidenum[j] == NO_INDEX) I_Error("Saved side does not exist");
                side_t *side = &sides[line->sidenum[j]];
...
void P_ArchiveKeyframe(json_mut_doc_t *doc, json_mut_t *root_mut)
{
    ...
    JS_SetInt(doc, root_mut, "native_musmap", writep_mobj(musinfo.mapthing));
    JS_SetInt(doc, root_mut, "native_muslast", writep_mobj(musinfo.lastmapthing));
    EndArchive();
}
...
void P_UnArchiveKeyframe(json_t *root)
{
    ...
    musinfo.mapthing=readp_mobj(JS_GetIntegerValue(root,"native_musmap"));
    musinfo.lastmapthing=readp_mobj(JS_GetIntegerValue(root,"native_muslast"));
    // Rebuild pointers into the newly allocated thinker arena.
    P_SpawnBrainTargets();
    EndUnArchive();
}

// Validate native keyframe topology before any arena is replaced. References
// decoded later also pass checked array-index and actor-class accessors above.
static int native_range(json_t *obj,const char *key,int low,int high) {
    int value=JS_GetIntegerValue(obj,key);
    if(value<low || value>high)I_Error("Invalid saved %s",key);
    return value;
}
static void native_array(json_t *obj,const char *key,int count) {
    if(JS_GetArraySize(JS_GetObject(obj,key))!=count)I_Error("Invalid saved %s array",key);
}
void ME_ValidateKeyframe(json_t *root) {
    json_t *thinkers=JS_GetObject(root,"thinkers");int count=JS_GetArraySize(thinkers);
    if(count<1 || count>200000)I_Error("Invalid saved thinker count");
    byte *seen=calloc(count,1);if(!seen)I_Error("Save allocation failed");
    int cursor=JS_GetIntegerValue(JS_GetObject(root,"thinkercap"),"next"),previous=head_index;
    while(cursor!=head_index) {
        if(cursor<0 || cursor>=count || seen[cursor])I_Error("Invalid saved thinker chain");
        seen[cursor]=1;
        json_t *wrapper=JS_GetArrayItem(thinkers,cursor),*data=JS_GetObject(wrapper,"thinker");
        int type=native_range(wrapper,"class",tc_mobj,tc_none);
        json_t *links=type==tc_none ? data:JS_GetObject(data,"thinker");
        if(JS_GetIntegerValue(links,"prev")!=previous)I_Error("Broken saved thinker links");
        previous=cursor;cursor=JS_GetIntegerValue(links,"next");
    }
    if(JS_GetIntegerValue(JS_GetObject(root,"thinkercap"),"prev")!=previous)I_Error("Incomplete saved thinker chain");
    // Freed arena slots can survive until the next collection after their
    // thinker has already been unlinked. Only removal functions permit this.
    for(int i=0;i<count;i++)if(!seen[i]) {
        int type=JS_GetIntegerValue(JS_GetArrayItem(thinkers,i),"class");
        if(type!=tc_mobj_del && type!=tc_ceiling_del && type!=tc_door_del &&
           type!=tc_floor_del && type!=tc_plat_del && type!=tc_elevator_del)
            I_Error("Unlinked active saved thinker");
    }
    memset(seen,0,count);
    for(int c=0;c<NUMTHCLASS;c++) {
        int sentinel=tclass_to_index[c];
        json_t *cap=JS_GetArrayItem(JS_GetObject(root,"thinkerclasscaps"),c);
        cursor=JS_GetIntegerValue(cap,"cnext");previous=sentinel;
        while(cursor!=sentinel) {
            if(cursor<0 || cursor>=count || seen[cursor])I_Error("Invalid saved thinker class chain");
            seen[cursor]=1;
            json_t *item=JS_GetArrayItem(thinkers,cursor),*body=JS_GetObject(item,"thinker");
            json_t *links=JS_GetIntegerValue(item,"class")==tc_none ? body:JS_GetObject(body,"thinker");
            if(JS_GetIntegerValue(links,"cprev")!=previous)I_Error("Broken saved class links");
            previous=cursor;cursor=JS_GetIntegerValue(links,"cnext");
        }
        if(JS_GetIntegerValue(cap,"cprev")!=previous)I_Error("Invalid saved class tail");
    }
    free(seen);
    json_t *wrapper;
    JS_ArrayForEach(wrapper,thinkers) {
        int type=native_range(wrapper,"class",tc_mobj,tc_none);
        json_t *data=JS_GetObject(wrapper,"thinker");
        if(type==tc_mobj || type==tc_mobj_del) {
            int actor=native_range(data,"type",0,num_mobj_types-1);
    ...
    json_t *player=JS_GetArrayItem(JS_GetObject(root,"players"),0);
    native_range(player,"playerstate",PST_LIVE,PST_LIVE);native_range(player,"health",1,INT_MAX);
    native_range(player,"mo",0,count-1);native_range(player,"readyweapon",0,NUMWEAPONS-1);native_range(player,"pendingweapon",0,wp_nochange);
    native_array(player,"ammo",NUMAMMO);native_array(player,"maxammo",NUMAMMO);native_array(player,"weaponowned",NUMWEAPONS);
    native_array(player,"cards",NUMCARDS);native_array(player,"powers",NUMPOWERS);native_array(player,"psprites",NUMPSPRITES);native_array(player,"frags",MAXPLAYERS);
    json_t *levels=JS_GetObject(player,"visitedlevels"),*level;
    if(JS_GetArraySize(levels)>32)I_Error("Too many saved visited levels");
    JS_ArrayForEach(level,levels){native_range(level,"episode",1,1);native_range(level,"map",1,32);}
    // Array/object sizes are resource-derived, never taken from the file to
    // allocate engine map buffers. Only player zero is accepted.
    for(int i=1;i<MAXPLAYERS;i++)if(JS_GetObjectSize(JS_GetArrayItem(JS_GetObject(root,"players"),i)))I_Error("Multiplayer save unsupported");
    json_t *rngobj=JS_GetObject(root,"rng");native_array(rngobj,"seeds",NUMPRCLASS);
    json_t *sectors_arr=JS_GetObject(root,"sectors"),*sector;
    JS_ArrayForEach(sector,sectors_arr) {
        native_range(sector,"floorpic",0,numflats-1);native_range(sector,"ceilingpic",0,numflats-1);
    }
    json_t *dirty;
    JS_ArrayForEach(dirty,JS_GetObject(root,"dirty_lines")){native_range(dirty,"line",0,numlines-1);}
    JS_ArrayForEach(dirty,JS_GetObject(root,"dirty_sides")){native_range(dirty,"side",0,numsides-1);}
}
