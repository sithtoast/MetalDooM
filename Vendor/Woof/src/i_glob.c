...
static MEFS_EnumerationResult Callback(void *userdata, const char *dirname,
                                      const char *fname)
{
...
static void ReadAllFilenames(glob_t *glob)
{
    ...
    if (!MEFS_EnumerateDirectory(glob->directory, Callback, glob))
    {
