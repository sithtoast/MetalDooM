...
python3 "$PROJECT_DIR/Tests/make_interpolation_fixture.py" "$OUT/fixtures"
xcrun swiftc -swift-version 5 -O -target arm64-apple-macosx14.0 -module-cache-path "$PROJECT_DIR/build/module-cache" \
 ...
 "$PROJECT_DIR/Tests/ExtendedInterpolationValidation.swift" -o "$OUT/validate-interpolation"
"$OUT/validate-interpolation" "$1" "$OUT/MetalDooMWorker"
