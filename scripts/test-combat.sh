...
TEST_DIR="$(mktemp -d "$PROJECT_DIR/build/combat-check.XXXXXX")"
...
xcrun clang -DMD_TESTING -I"$PROJECT_DIR/Engine" -arch arm64 -mmacosx-version-min=14.0 \
  "$PROJECT_DIR/Tests/CombatValidation.c" "$TEST_DIR/objects/libDoom.a" \
  -Wl,-dead_strip -o "$TEST_DIR/validate-combat"
"$TEST_DIR/validate-combat" "$WAD_PATH"
