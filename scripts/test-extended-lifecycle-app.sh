...
extension ExtendedPreviewApp {
    ...
    var validationBoundaryControls:Bool {!clock.busy && !runButton.isEnabled && restartButton.isEnabled && musicPlayer?.isPlaying==false}
...
for fixture in normal death; do
 ...
 "$OUT/validate" --rust-preview "$1" --map MAP01
done
