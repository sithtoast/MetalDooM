...
OUT="$PROJECT_DIR/build/lifecycle-app-test"
mkdir -p "$OUT"
...
(out/'main.swift').write_text(main+'\n'+(root/'Tests/ExtendedLifecycleAppValidation.swift').read_text())
s=(root/'Sources/ExtendedPreviewApp.swift').read_text().replace('plan.paths','(plan.paths+[URL(fileURLWithPath:ProcessInfo.processInfo.environment["LIFECYCLE_FIXTURE"]!)])')
...
for scenario in normal:1 death:1 normal:7 normal:14; do
 fixture="${scenario%:*}"
 map="${scenario#*:}"
