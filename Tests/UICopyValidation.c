...
int main(int argc,char **argv){
 ...
 assert(ME_Init(&config));size_t size=ME_CopyUI(NULL,0);assert(size==92);
 unsigned char data[108];memset(data,0xa5,sizeof(data));assert(ME_CopyUI(data,91)==92);
 for(int i=0;i<108;i++)assert(data[i]==0xa5);
 assert(ME_CopyUI(data,92)==92 && !memcmp(data,"MUI1",4));
 for(int i=92;i<108;i++)assert(data[i]==0xa5);
 ...
 assert(ME_CopyUI(data,92)==92 && word(data+16)==100 && (word(data+28)&3)==3);
