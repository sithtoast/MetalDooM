...
int main(int argc,char **argv) {
    assert(argc==3 && ME_CopyPresentation(NULL,0)==0 && ME_CopyMaterials(NULL,0)==0);
    ...
    assert(ME_Init(&c));
    size_t (*copies[])(void*,size_t)={ME_CopyPresentation,ME_CopyMaterials};
    ...
    free(data);free(copy); } puts("PASS sprite/material snapshots complete-copy bounds, stable values and canaries");
}
