...
with tempfile.TemporaryDirectory() as cache:
    ...
    for mode in ['sequence', 'length', 'operation', 'truncated', 'reserved', 'quit', 'eof', 'action', 'action-length', 'campaign-playing', 'campaign-length', 'save-length', 'blend-length', 'restore-empty', 'restore-oversize', 'restore-truncated']:
        ...
        try:
            ...
            assert len(sprite_data) == presentation and sprite_data[:4] == b'MSP6'
            ...
            assert len(initial[2]) == 56+geometry+presentation+materials+audio+ui and initial[2][56+geometry+presentation:][:4] == b'MMT2'
            if mode == 'eof':
                p.stdin.close()
