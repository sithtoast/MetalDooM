...
p,size,_=lumps[m+1]
poses={'exit':(3008,-4256,0),'sky':(2000,-3540,90),'progression':(2944,-4768,180),'pillar':(288,-3040,270),'ceiling':(2848,-2960,45),'light':(384,-3120,225),'effects':(384,-3120,225),'advanced':(384,-3120,225),'emission':(2848,-2960,270)}
...
if scene in ('effects','advanced'):
 # Append a replacement THINGS lump; retain all original geometry and art.
 things=bytes(b[p:p+size])+b''.join(struct.pack('<hhhHH',tx,ty,0,kind,7)
     for tx,ty,kind in [(300,-3192,44),(432,-3256,46),(344,-3352,45)]+([(304,-3288,2012),(256,-3288,2035)] if scene=='advanced' else []))
 struct.pack_into('<ii',b,o+(m+1)*16,len(b),len(things))
 b.extend(things)
