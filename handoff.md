...
Engine view/weapon bob was already present. Run now interpolates camera and
compatible weapon positions; no vertical look is added. Manual buttons retain their prior
...
The native audio `present` path accepts the initial or restored tic, then consecutive single tics, applies
...
back ceiling name60, actor clip limits68/72. Names/lights and both full/cached
packets validate. Other packets and ABI2's eighteen exports stay unchanged.

RenderSector.c adapts pinned Woof R_FakeFlat without mutating physics; the copied
view and sector resolver share the tic-zero camera calculation. Front/back water
rules, above-ceiling/sky cases, flat offsets, relative/absolute transferred lights
...
interpolation and full campaign/boss playtesting. These are Boom fake sectors,
not arbitrary stacked 3D floors; sector presentation remains discrete per tic.
...
Previous candidate: `build/wall-palette-preview/MetalDooM.app`, 0.10.0/build151;
...
flag16 (additive, requires translucent8). Shadow wins; unsupported per-state/
per-object custom tables and malformed TRANMAP fail explicitly. Native sprites
retain palette indices and blend back-to-front with depth testing, no depth writes,
and cutouts. RGB world/shaded sprite colors are quantized to PLAYPAL before table
...
Worker/protocol/all-map tests: `build/translucency-worker-validation.log`.
Native GPU pixels: `build/translucency-metal-validation.log`; private save
regressions: `build/translucency-save-validation.log`. Remaining work includes
custom/wall translucency, palette powerups, fake-floor/control-sector/sky effects,
actor/moving-surface interpolation and actual campaign/boss acceptance.

**Build 148 smooths camera and weapon presentation during Run.** Basic engine
view/weapon bob already existed; earlier notes overstated that gap. MSP2/version2
uses header word28 bit0 to snap the camera from the engine interpolation marker,
including short teleports. Strides, MVW5/MGE2 and17 exports stay unchanged.
ExtendedInterpolation in ExtendedPresentation.swift blends consecutive live-map
poses for one tic from receipt, shortest-path yaw, compatible weapon/flash slots.
No extrapolation; late replies, map/tic jumps, teleport flags and large corrections
snap. Pause clears history and displays the latest engine pose; manual steps stay
exact. New renderers on restart/Continue/Load start without history. Actors,
animation frames, world surfaces, HUD and audio still update at original tic timing.
No geometry/texture uploads or simulation requests occur during interpolation.

Previous candidate: `build/interpolation-final/MetalDooM.app`, 0.10.0/build 148;
`build/build148.log`. Host deep/strict signatures and plist/running title pass.
Actual MAP01 Run/Up/Right/Pause is verified; left paused at tic202/health100/ammo50,
Music/Sound on, with world/pistol/HUD screenshot checked. Preserve build146 and all earlier bundles/saves, plus intermediate build 147 at
`build/interpolation-preview`. Final 148 makes wrapped-yaw endpoints exact. Tests:
`build/interpolation-validation.log`, `build/interpolation-metal.log`,
`build/interpolation-native.log`, `build/interpolation-worker-regression.log`,
`build/interpolation-save-regression.log` and `build/interpolation-clock.log`.
See docs/EXTENDED_INTERPOLATION.md and docs/VALIDATION.md for timing, host evidence
and remaining rendering/playthrough limits. Old saves require their matching
preserved engine after the MSP2 engine fingerprint change.
...
Previous candidate: `build/scroll-preview/MetalDooM.app`, 0.10.0/build146;
...
actor/moving-surface interpolation, palette/translucency, control-sector/fake-floor/sky effects.
...
pin, with native strict/index/topology validation, MUSINFO pointers and brain target
rebuilding. SaveSnapshot.c adds native clocks/counters/material translations/UI.
The `.mdrust` MRS1 envelope hashes the engine library and payload; JSON includes
ordered WAD identity. Atomic writes; load validation and candidate resource
preparation happen before worker swap. Failed loads/file writes preserve the old
paused game. Inputs and sound tails clear, the selected music restarts, and the
...
that restored state is left open. Private save: build/save-test/build145-MAP01.mdrust.
Keep build 144 and older candidates intact. See `docs/VALIDATION.md` for details. `build/save-validation.log`
covers all 16 maps, both Rust weapon fixtures, Use, five post-transition pickup/
history cases, exact restored scene state and 140 future tics, malformed JSON,
restore boundaries, copied-state canaries and file-envelope checks.
`build/save-native.log` covers repeated worker replacements, subsequent audio,
paused controls/music, corrupt save/write failures and close during restore.
`build/save-core.log` and `build/save-worker-regression.log` cover 17 exports,
core behavior, new protocol bounds and existing native worker regressions.
Full actual campaign/boss playthrough acceptance is next; keep the picker guard.
...
then load fresh paused world state. No automatic episode selection; save/restore is described above.
...
Previous candidate: `build/campaign-preview/MetalDooM.app`, 0.10.0/build 144,
...
fresh HUD/world. That candidate was verified Paused, Music/Sound on. Preserve prior143 at `build/lifecycle-complete` and all older
...
G_DoWorldDone in the existing worker; inventory and RNG stay engine-owned. The
startup now sets haswolflevels from MAP31 presence, matching upstream; otherwise
commercial secret exits silently take normal routes. Continue disables upstream's
default-on autosave; explicit native saves do not enable it.

MUI2/version 2 is now 144 bytes. Original HUD/music offsets stay; offsets 92–140 add
phase, current/next map, kills/items/secrets/totals, time and secret-exit flag.
MVW5/header56 and other payloads remain. MEQ1 op4 carries u32 action0/1 and forces
complete tic-zero geometry. Tick batches stop at death/completion. Old UI rejects;
Swift validates expected new map, phase, full geometry and stable resource identity.

The native death panel offers Restart level or E/Space/Return. Completion displays
stats/time/next map; Continue carries health/armor/weapons/ammo, clears keys, and
loads a fresh native renderer/builder/audio player. Restart resets inventory and
...
reset world/pistol HUD and working toolbar. Preserve 141 at build/lifecycle-preview,
142 at build/lifecycle-final, and all earlier candidates.

`lifecycle-validation.log`: actual exit switches in original rooms, unmodified Rust
UMAPINFO, all 16 normal routes plus MAP02→15 and MAP10→16 secret routes, both returns
and episode endings, pickups/carryover/key cleanup, new counters/geometry and repeated
restart. Damaging-floor death at 129 restarts to health 100/tic 0. These are route probes,
not complete campaign/boss playthroughs. `lifecycle-native.log`: actual app source
with injected fixture resources, completion/death controls, MAP02 D_SHORES music,
repeated restart/step and close during pending level load. `lifecycle-worker.log`,
`lifecycle-ui.log` and `lifecycle-core-regression.log` cover worker/core/all-map/
24 malformed UI/MIDI/copy regressions. See VALIDATION.md for details and limits.
...
Previous candidate `build/ui-preview/MetalDooM.app`, **0.10.0/build 140**, log
`build/build140.log`. Host deep/strict signature and bundled version pass; native
CUA title agrees. MAP01 displays health 100/armor 0/PISTOL/BULLETS50; a 35-tic step
then music-muted firing ends at tic70/ammo47, Sound on. Music restored, Run +
weapon1 then Escape ends at tic179, FIST with no ammo group, health 100/armor 0,
...
Native music PCM peak 0.07998366; physical speaker hearing remains unverified.
...
across maps. Actor/moving-surface interpolation, palette/translucency/control-sector/
...
MUI2 carries HUD/music/lifecycle; existing structures and MGE2/MSP2/MMT1/MSA1 stay.
MVW5 header56 includes UI size at52; payloads are optional MGE2 followed by required
MSP2/MMT1/MSA1/MUI2. Old outer versions reject. `ME_EnableAudio` precedes
...
load its dylib in the Swift app.
